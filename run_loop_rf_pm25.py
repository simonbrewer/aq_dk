...
## -----------------------
## Air monitor data
aq_df = pd.read_csv("./data/loop_test/summer23_pm_stationary.csv")
...
for i in all_days:
    ...
    if (phi_2_reduce.shape[0] > 1):
        ...
        ## Predict for EBus
        X_pred = phi_2_reduce
        ebus2_df_sub['yhat'] = aq_rf.predict(X_pred)
        ebus2_df_sub['adj'] = ebus2_df_sub['val'] - ebus2_df_sub['yhat']

        ## Write out
        print(i)
        ebus2_df_sub.to_csv("./output/pm25/adj_" + str(target_date.date()) + ".csv", sep=',', index=False)
