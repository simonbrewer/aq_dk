...
for site in aq_sites:
    ...
    for i in all_days:
        ...
        if (phi_2_reduce.shape[0] > 1):
            ...
            ## Create and train
            aq_rf = RandomForestQuantileRegressor()
            aq_rf.fit(X_train, y_train)

            ## Predict for EBus
            X_pred = phi_2_reduce
            test_pred = aq_rf.predict(X_pred, quantiles=[0.025, 0.5, 0.975])
            test_sub['yhat'] = test_pred[:,1]
            test_sub['yhat_lo'] = test_pred[:,0]
            test_sub['yhat_hi'] = test_pred[:,2]
            test_sub['adj'] = test_sub['sample.measurement'] - test_sub['yhat']

            ## Write out
            print(i)
            test_sub.to_csv("./xv_output/o3_qrf_2023/" + str(site) + "_adj_" + str(target_date.date()) + ".csv", sep=',', index=False)
